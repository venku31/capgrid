...
def scan_lot(lot_no,warehouse_location,company):
    stock = frappe.db.sql("""SELECT            
        sle.lot_number,sle.item_code,sle.stock_uom,sle.warehouse_location,sum(sle.actual_qty) as actual_qty
        FROM `tabStock Ledger Entry` sle
...
def picklist_item_query(doctype, txt, searchfield, start, page_len, filters):
    if filters.get("parent"):
        return frappe.db.sql(""" select lot_number,to_be_picked,parent,part_number_name from `tabPickList Details`
