...
function total_qty(frm, cdt, cdn) {
	...
	frm.doc.details.forEach(function(d) { total_qty += d.qty});
    frm.doc.details.forEach(function(d) { total_picked_qty += d.picked_qty});
...
function fetch_actual_qty(frm, cdt, cdn) {
    $.each(frm.doc.details || [], function(i, d) {
        ...
        frappe.call({
                ...
                callback: function(r) {
                    console.log(r.message);
                    var actual_qty = r.message[0].actual_qty
